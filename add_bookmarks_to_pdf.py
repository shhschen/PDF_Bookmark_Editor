...
class BookmarkGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("PDF 書籤添加工具（支援層級）")
        ...
        tk.Label(self.entry_frame, text="層級", width=8).grid(row=0, column=0)
        tk.Label(self.entry_frame, text="頁碼 (從1)", width=12).grid(row=0, column=1)
        tk.Label(self.entry_frame, text="書籤標題", width=40).grid(row=0, column=2)
        ...
        tk.Button(root, text="從文字檔匯入書籤 (格式: 層級\t頁碼\t標題)", command=self.import_from_txt).pack(pady=5)
    ...
    def add_entry(self, level_val="0", page_val="", title_val=""):
        ...
        tk.Entry(self.entry_frame, textvariable=level_var, width=5).grid(row=self.entry_row, column=0, padx=2, pady=2)
        tk.Entry(self.entry_frame, textvariable=page_var, width=8).grid(row=self.entry_row, column=1, padx=2, pady=2)
        tk.Entry(self.entry_frame, textvariable=title_var, width=40).grid(row=self.entry_row, column=2, padx=2, pady=2)
    ...
    def import_from_txt(self):
        ...
        try:
            ...
            if failed_lines:
                error_msg = "下列行格式錯誤 (需為 層級<TAB>頁碼<TAB>標題):\n" + "\n".join(failed_lines)
    ...
    def load_bookmarks_from_pdf(self):
        ...
        if not path:
            messagebox.showerror("錯誤", "請先選擇 PDF 檔案")
            return
        try:
            reader = PdfReader(path)
            ...
            messagebox.showinfo("完成", "已載入 PDF 書籤")
        except Exception as e:
            messagebox.showerror("錯誤", f"讀取 PDF 書籤失敗：{e}")

    def clear_all_entries(self):
        for widget in self.entry_frame.winfo_children()[3:]:
            widget.destroy()
        self.entries.clear()
        self.entry_row = 1
    ...
    def export_template(self):
        ...
        with open(path, "w", encoding="utf-8") as f:
            f.write("""# 範例格式：層級<TAB>頁碼<TAB>書籤名稱\n""")
            f.write("0\t1\t第一章\n")
            f.write("1\t2\t1.1 小節\n")
            f.write("1\t3\t1.2 小節\n")
            f.write("0\t4\t第二章\n")
            f.write("1\t5\t2.1 小節\n")
        messagebox.showinfo("匯出成功", f"模板已儲存於：{path}")
    ...
    def apply_bookmarks(self):
            ...
            try:
                ...
                if title and 0 <= page < len(reader.pages):
                    while len(stack) > level:
                        stack.pop()
                    parent = stack[-1] if level > 0 and len(stack) >= level else None
        ...
        messagebox.showinfo("完成", f"書籤已寫入：{output_path}")
